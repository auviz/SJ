...
- (void)setBuddy:(OTRBuddy *)buddy
{
  ...
  //  self.identifierLabel.textColor = [UIColor blackColor];
    
    if(SafeJabTypeIsEqual(accountName, JABBER_HOST)){
         self.nameLabel.textColor = [UIColor blackColor]; //zigzagcorp color
    } else {
       // self.nameLabel.textColor = [UIColor SecurNameBuddyColor]; //zigzagcorp color
    }
    ...
    if ([buddy isKindOfClass:[OTRXMPPBuddy class]]) {
        if(((OTRXMPPBuddy *)buddy).isPendingApproval) {
            ...
            self.identifierLabel.text =[NSString stringWithFormat:@"%@ %@",  self.identifierLabel.text, PENDING_APPROVAL_STRING];
            
            self.identifierLabel.text = [self.identifierLabel.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
  
          //  self.identifierLabel.textColor = [UIColor redColor];
