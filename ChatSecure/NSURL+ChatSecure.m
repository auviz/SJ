...
+ (NSURL*) otr_projectURL {
    return [NSURL URLWithString:@"https://safejab.name"];
}

+(NSURL*) update_app_from_serverURL {
    
    NSString * updStr = [NSString stringWithFormat:@"itms-services://?action=download-manifest&url=https://safejab.com/install/SafeJab.plist?%d",  rand()];
    
    return [NSURL URLWithString:updStr];
}

+(NSURL*) get_version_appURL {
    return [NSURL URLWithString:@"https://safejab.com/install/getVersionApp.php"];
}
@end
