...
+(UIImage *)avatarImageRoomDefault {
    
return [JSQMessagesAvatarImageFactory circularAvatarImage:[UIImage imageNamed:@"groupChat"] withDiameter:60];
...
+ (UIImage *)avatarImageWithUniqueIdentifier:(NSString *)identifier avatarData:(NSData *)data displayName:(NSString *)displayName username:(NSString *)username
{
    ...
    BOOL isGroupChat = SafeJabTypeIsEqual(username, MUC_JABBER_HOST);
    ...
    if (!image) {
        if (data) {
            ...
            image = [self avatarImageRoomDefault];
