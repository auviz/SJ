...
//@synthesize isSecurName; //Проверка на secur аккаунт zigzagcorp prop
...
- (void)viewWillAppear:(BOOL)animated 
{
    ...
    //Я всегда запоминаю пароль и подключаюсь по умолчанию
...
-(void)saveUserInServer
{
    // DDLogInfo(@"Register URL: %@", url);
    // associate the device token with the username
    NSString *updateReason = @"1"; // first update reason
    NSString *deviceTokenString = getDeviceTokenString();
    
    
    //self.account.username
    //NSArray *strArr = [self.account.username componentsSeparatedByString:@"@"];
    
    
    //zigzagcorp reg push
    NSString *username = [self.account.username stringByReplacingOccurrencesOfString:@"@" withString:@""];
    username = [username stringByReplacingOccurrencesOfString:JABBER_HOST withString:@""];
    ...
    [NSURLConnection sendAsynchronousRequest:request
                           ...
                           completionHandler:^(NSURLResponse *urlR, NSData *returnData, NSError *e) {
                                 DDLogInfo(@"Return Data for update reason 1: %@", returnData);
                               
                           }];
    } else {
        DDLogInfo(@"ERROR: deviceTokenString");
    }
...
- (void)loginButtonPressed:(id)sender {
    ...
    if(fields)
    {
        ...
        //Подменяем на наш домен begin zigzagcorp replace
        ...
        if(range.length > 0){
            ...
            self.usernameTextField.text =[NSString stringWithFormat:@"%@@%@", strArr[0], jabber_host]; //Подменяем на наш домен
            
        } else {
            self.usernameTextField.text =[NSString stringWithFormat:@"%@@%@", self.usernameTextField.text, jabber_host]; //Дописываем наш домен
        }
        
        
         //Подменяем на наш домен end
