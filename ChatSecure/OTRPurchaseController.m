...
- (void) productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response {
    if ([response.invalidProductIdentifiers count] > 0) {
        DDLogInfo(@"Invalid products identifiers: %@", [response.invalidProductIdentifiers description]);
    }
...
- (void)paymentQueue:(SKPaymentQueue *)queue updatedTransactions:(NSArray *)transactions {
    for (SKPaymentTransaction *transaction in transactions) {
        DDLogInfo(@"Transaction: %@", transaction.transactionIdentifier);
        switch (transaction.transactionState) {
            case SKPaymentTransactionStatePurchased:
                DDLogInfo(@"Transaction purchased");
            ...
            case SKPaymentTransactionStateFailed:
                DDLogInfo(@"Transaction failed");
                [self sendProductUpdateNotification];
                [[SKPaymentQueue defaultQueue] finishTransaction: transaction];
                break;
            case SKPaymentTransactionStateRestored:
                DDLogInfo(@"Original transaction restored: %@", transaction.originalTransaction.transactionIdentifier);
                //[[OTRPushController sharedInstance] registerWithReceipt:transaction.transactionReceipt resetAccount:YES];
                [self setProductIdentifier:transaction.payment.productIdentifier purchased:YES];
                [self sendProductUpdateNotification];
                [[SKPaymentQueue defaultQueue] finishTransaction: transaction];
                DDLogInfo(@"Transaction restored");
                break;
            case SKPaymentTransactionStatePurchasing:
                DDLogInfo(@"Purchasing transaction... ");
                break;
            default:
                break;
        }
...
- (void) saveProductsDictionary:(NSMutableDictionary*)productsDictionary {
    ...
    if (!success) {
        DDLogInfo(@"Product preferences not saved to disk!");
    }
}
